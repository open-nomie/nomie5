<script lang="ts">
  ...
  $: if (term) {
    results = Object.keys($PeopleStore.people)
      .filter((username: string) => {
        ...
        return (
          `${person.displayName} ${person.username} ${person.notes}`
            .toLowerCase()
            .search(term.replace('@', '').toLowerCase()) > -1
        )
      })
      .map((username) => {
  ...
  $: if (
    $SearchStore.active &&
    $SearchStore.active.term &&
    $SearchStore.active.term !== term
  ) {
...
<section class="pt-2 n-panel stiff">
  <SearchBar
    compact
    autofocus
    showClose={true}
...
<section class="n-panel scroll-y column">
  {#if results.length}
    <main class="grid grid-cols-3 gap-2 p-4">
  ...
  {:else if term && !results.length}
    <ListItem
      title={Lang.t('search.no-results', 'No results found')}
      className="mt-4" />
  {/if}
