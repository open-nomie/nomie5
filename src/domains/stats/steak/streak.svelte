<script lang="ts">
  ...
  async function main() {
    if (element) {
      mockTracker =
        element.type == 'tracker'
          ? TrackerStore.getByTag(element.id)
          : new TrackerConfig({ tag: `${element.id}-mock` })
      let logs = await StreakHelper.getLogs(
        element,
        selectedDate,
        view,
    ...
    {:else if view == 'quarter'}
      <Row className="align-items-start">
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(2, 'month')}
          events={calendarLogs} />
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(1, 'month')}
          events={calendarLogs} />
        <Calendar
          tracker={mockTracker}
          compact
    ...
    {:else if view == 'year'}
      ...
      <Row className="align-items-start">
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(3, 'month')}
          events={calendarLogs} />
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(4, 'month')}
          events={calendarLogs} />
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(5, 'month')}
          events={calendarLogs} />
      </Row>
      <Row className="align-items-start">
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(6, 'month')}
          events={calendarLogs} />
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(7, 'month')}
          events={calendarLogs} />
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(8, 'month')}
          events={calendarLogs} />
      </Row>
      <Row className="align-items-start">
        <Calendar
          tracker={mockTracker}
          compact
          initialDate={selectedDate.subtract(9, 'month')}
          events={calendarLogs} />
        <Calendar
          tracker={mockTracker}
          compact
