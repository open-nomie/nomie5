<script lang="ts">
  ...
  import BoardSortModal from '../../domains/board/board-sort.svelte'
...
<NLayout pageTitle={appTitle}>
  <header slot="header">
    <Container>
      <Toolbar>
        ...
        <Button
          icon
          className="board-option-action text-primary-500"
  ...
  <!-- end header-->
  <div slot="content" class="pt-4 bg-gray-100 board-container dark:bg-gray-800">
    {#if $UserStore.storageType == 'blockstack'}
      <div class="my-1 blockstack-warning">
    ...
    {#if user}
      ...
      {:else}
        {#if daysSinceLastBackup > 6 && $UserStore.launchCount > 10 && $UserStore.storageType == 'local' && $UserStore.meta.hideBackup == false}
          <div class="flex items-center justify-center container-sm">
            <button
              aria-label="Backup your data"
              on:click={exportData}
              class="flex items-center px-3 py-2 mb-2 text-sm text-center bg-gray-400 rounded-full backup dark:bg-black dark:text-white bg-opacity-20">
              ...
              {/if}
              <div class="ml-2 font-bold text-black dark:text-white">
        ...
        {#if (foundTrackers || boardTrackers || []).length}
          <div class="flex mt-5 mb-3 board-actions" style="min-width:100px;">
