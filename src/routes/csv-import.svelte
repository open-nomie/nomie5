<script lang="ts">
  ...
  $: if (
    activeImporter &&
    activeImporter.parsed &&
    activeImporter.parsed.data
  ) {
  ...
  function mapField(fieldIndex) {
    ...
    Interact.popmenu({
      title: `Map ${
        activeImporter ? activeImporter.getHeaders()[fieldIndex] : 'Unknown'
      }`,
  ...
  function getIndexMap(index) {
    ...
    if (
      activeImporter &&
      activeImporter.config &&
      activeImporter.config.fieldMap
    ) {
  ...
  async function startImport() {
    ...
    if (confirmed) {
      if (activeImporter.length() > 1000) {
        Interact.toast(
          `Importer starting. With this many records it might freeze the UI, just give it time...`,
          { perm: true },
        )
      } else {
  ...
  async function insertField() {
    let selectedField: any = await _selectField(
      `Select a field to Insert into the note`,
    )
    if (selectedField) {
      activeImporter.config.template = `${
        activeImporter.config.template || ''
      }{f${selectedField.index}}`.trim()
    }
  }
...
<Layout className="import" showTabs={false}>
  ...
  <main slot="content" class="page page-csv-import flex-column">
      ...
      <div class=" p-0 import-csv-home">
        ...
        <ListItem>
          <Text size="md" leading3 bold>
            <span class="nbtn nbtn-xs nbtn-danger nbtn-rounded">
              {Lang.t('general.beta', 'Beta')}
            </span>
            Warning
          </Text>
          <Text size="sm" faded>
            This CSV importer is a work-in-progress and results may vary. PLEASE
        ...
        <ListItem itemDivider compact bg="transparent">
          Create an Importer
        </ListItem>
        <ListItem>
          <FileUploader
        ...
        <ListItem bg="transparent">
          <Text size="sm" faded>
            Select any CSV to begin. You can then map the appropriate fields
          </Text>
        </ListItem>
    ...
    -->
      <div class=" p-0 import-csv-note">
        <ListItem bottomLine>
          <Input
            type="text"
            bind:value={activeImporter.config.name}
    ...
    {#if previewLog && is.truthy(previewIndex) && activeImporter.parsed}
      ...
      <ListItem>
        <Button block on:click={startImport}>
